...
use ::subs::{
    consts::*, gaussian_filter::GaussianFilter, get_input, judge::Judge,
...
impl Solver {
    ...
    fn create_temperature(&mut self) {
        ...
        // 3x3ガウシアンフィルタで平滑化
        self.temperature.smoothing3x3();
    }

    /// i番目の出口の座標を特定する
    /// - 出口を中心とした3x3のパターンを取得する
    fn get_3x3_pattern(&self, i: usize) -> usize {
        let pattern = vec![
            vec![
                Judge::measure(i, -1, -1),
                Judge::measure(i, 0, -1),
    ...
    fn predict(&mut self) -> Vec<usize> {
        // 予測結果を保存する配列
        (0..self.N).map(|i| self.get_3x3_pattern(i)).collect()
    }
}
