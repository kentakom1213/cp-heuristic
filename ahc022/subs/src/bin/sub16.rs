...
impl Solver {
        ...
        for r in 0..self.L {
            for c in 0..self.L {
                if !mask[r][c] {
                    ...
                    if self.S < 500 {
                        if self.temperature.field[r][c] < FILL {
                            self.temperature.field[r][c] = 300;
                        } else if self.temperature.field[r][c] > FILL {
                            self.temperature.field[r][c] = 700;
                        }
                    } else {
                        // S >= 400のとき、0 | 1000のどちらかに
                        if self.temperature.field[r][c] < FILL {
                            self.temperature.field[r][c] = 0;
                        } else if self.temperature.field[r][c] > FILL {
                            self.temperature.field[r][c] = 1000;
                        }
                    }
                }
            }
    ...
    fn measure_acculate(&self, i: usize, r: isize, c: isize) -> usize {
        if self.S < 80 {
            ...
            (0..SAMPLE).map(|_| Judge::measure(i, r, c)).sum::<usize>() / SAMPLE
        }
    }
