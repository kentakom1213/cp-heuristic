...
const SAMPLING_SPAN: usize = 30;
const SAMPLING_RANGE: usize = 2;
const WEIGHT_N: usize = 2;
const WEIGHT_M: usize = 1;
...
trait UsizeTools {
    ...
    fn powQ(&self, n: usize, m: usize) -> usize;
}
impl UsizeTools for usize {
    ...
    fn powQ(&self, n: usize, m: usize) -> usize {
        ...
        while (ng - ok) > 1 {
            let mid = (ok + ng) / 2;
            if mid.saturating_pow(m as u32) <= self.saturating_pow(n as u32) {
                ok = mid;
            } else {
                ng = mid;
            }
...
impl Field {
    ...
    /// ### 出力
    /// `Pos`：最も値の低い座標を返す
    fn sampling(&mut self, start: Pos, end: Pos, rng: &mut StdRng) {
    ...
    /// ## predict_edge_cost
    /// 辺に重み付けを行う
    fn predict_edge_cost(&mut self, start: Pos, end: Pos) -> usize {
        let (sr, sc) = start;
        let (er, ec) = end;
        let (mr, mc) = ((sr + er) / 2, (sc + ec) / 2);
        let cost = start.dist(&end).powQ(WEIGHT_N, WEIGHT_M)
                    * self.destroy((mr, mc)).powQ(WEIGHT_N, WEIGHT_M)
                    * self.destroy((er, ec));
        cost
    }
    ...
    fn get_shortest_path(&mut self, start: Pos, end: Pos, rng: &mut StdRng) -> Vec<Pos> {
        // 点群をサンプルに追加
        self.sampling(start, end, rng);
        let ALL: usize = self.samples.len();

        // スタート → { 点群 } → ゴール
        let mut points: Vec<Pos> = self.samples.iter().filter(|_| rng.gen_bool(0.5)).cloned().collect();
        points.push(end);
        let SIZ = points.len();
        ...
        let dist = {
            ...
            for u in 0..SIZ {
                for v in u..SIZ {
                    ...
                    let (er, ec) = points[v];
                    let (mr, mc) = ((sr + er) / 2, (sc + ec) / 2);
                    let cost = self.predict_edge_cost(points[u], points[v]);
                    d[u][v] = cost;
                    d[v][u] = cost;
                }
            }
        ...
        for (i, &p) in points.iter().enumerate() {
            ...
            let tmp_dist = self.predict_edge_cost(src, p) + dp[i][SIZ - 1];
