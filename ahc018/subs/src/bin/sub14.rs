...
impl Field {
    ...
    fn sampling(&mut self, d1: Pos, d2: Pos, rng: &mut StdRng) -> Pos {
        ...
        let sampling_h = height / SAMPLING_WIDTH;
        let sampling_w = width / SAMPLING_WIDTH;

        // サンプリング
        let (mut min_idx, mut min_cost) = (d1, INF);  // 答えとなる座標、掘るのにかかるコスト
        for i in 0..sampling_h {
            for j in 0..sampling_w {
                // 等幅でサンプリング
                let r = rstart + SAMPLING_WIDTH * i + SAMPLING_WIDTH / 2;
                let c = cstart + SAMPLING_WIDTH * j + SAMPLING_WIDTH / 2;
    ...
    fn tunnel_opt(&mut self, start: Pos, end: Pos, rng: &mut StdRng) {
        if start.dist(&end) < SAMPLING_WIDTH || self.sampling(start, end, rng) == start {
            // 直接掘り進める
            ...
            // 開始地からから経由地まで
            self.tunnel_opt(start, waypoint, rng);

            // 経由地から目的地まで
            self.tunnel_opt(waypoint, end, rng);
        }
    }
...
fn main() {
    ...
    while let Some((src, house)) = field.get_nearest_pair() {
        // 水源から家まで掘り進める
        field.tunnel_opt(src, house, &mut rng);
    }
}
