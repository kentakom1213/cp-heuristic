...
impl Field {
    ...
    /// ## tunnel_select2
    /// `start=(sr,sc)`から`end=(er,ec)`まで掘り進める
    /// - 鉛直→水平
    /// - 水平→鉛直
    /// をコストによって切り替える
    fn tunnel_select2(&mut self, start: Pos, end: Pos) {
        let (sr, sc) = start;
        let (er, ec) = end;
        ...
        let cost_p = match self.state[p.0][p.1] {
            State::Rock(_) => self.destroy(p),
            State::Destroyed(c) => c,
        };
        let cost_q = match self.state[q.0][q.1] {
            State::Rock(_) => self.destroy(q),
            State::Destroyed(c) => c,
        };

        if cost_p < cost_q {
            // 水平に掘り進める
            self.tunnel_h(sr, sc, ec);
            // 鉛直に掘り進める
            self.tunnel_v(ec, sr, er);
        } else {
            // 鉛直に掘り進める
...
fn main() {
    ...
    while let Some((src, house)) = field.get_nearest_pair() {
        // 水源から家まで掘り進める
        field.tunnel_select2(src, house);
    }
}
