...
const SAMPLING_RATE: usize = 20; // 1サンプリングあたりのマス数
...
impl Field {
    ...
    fn tunnel(&mut self, start: Pos, end: Pos) {
        ...
        // 掘削の終点を水源に追加
        self.sources.push((sr, sc));

        // 水平に掘り進める
        self.tunnel_h(er, sc, ec);
        // 掘削の終点を水源に追加
        self.sources.push((er, ec));
    }
    ...
    fn sampling(&mut self, d1: Pos, d2: Pos, rng: &mut StdRng) -> Pos {
        ...
        let sample_count = (height / SAMPLING_RATE) * (width / SAMPLING_RATE);
...
fn main() {
    ...
    for &house in &HOUSES {
        ...
        // サンプリングし、経由地を求める
        let waypoint = field.sampling(house, src, &mut rng);

        // 水源から経由地まで掘り進める
        field.tunnel(src, waypoint);

        // 経由地から家まで掘り進める
        field.tunnel(waypoint, house);
    }
}
